...
MOTOR_REAL_INFO can1motorRealInfo[7] = {0};//底盘4个3508电机m = 0 1 2 3;6020电机4 抬升电机5 凹轮电机6
MOTOR_REAL_INFO can2motorRealInfo[7] = {0};//履带2个3508电机m = 0 1; 夹爪2 云台3  6020电机4 抬升电机5 凹轮电机6
...
void Motor_Control(void)
{
    for(int i = 0; i < 7; i++)
    {
        ...
        switch (can1motorRealInfo[i].Motor_Mode)
        {
            ...
            case HOMEING_MODE://调用速度模式进行回零，检测停转一段时间后角度积分置零。
            {
                ...
                if(can1motorRealInfo[i].HomingMode.cnt >= 30) //计数30次
                {
                    ...
                    // Speed_Control(&can1motorRealInfo[i],0);
                }

                if (can1motorRealInfo[i].HomingMode.done_flag)      //已经回零完成
                {
                    pid_calc(&can1MOTOR_PID_RPM[i],0,can1motorRealInfo[i].RPM);//速度环置零
                }
                else
                {
                    pid_calc(&can1MOTOR_PID_RPM[i],can1motorRealInfo[i].TARGET_RPM,can1motorRealInfo[i].RPM);//速度环
        ...
        switch (can2motorRealInfo[i].Motor_Mode)
        {
            ...
            {
                //电流值小于回零状态时的电流值，计数器清零，大于则一直计数，连续计数到30次则回零完成
                ...
                //    Speed_Control(&can2motorRealInfo[i],0);
                }
                if (can2motorRealInfo[i].HomingMode.done_flag)      //已经回零完成
                {                                                                                               
                    pid_calc(&can2MOTOR_PID_RPM[i],0,can2motorRealInfo[i].RPM);     //速度环置零  
                    can2motorRealInfo[i].TARGET_RPM=0;
                }
                else
                {
                    pid_calc(&can2MOTOR_PID_RPM[i],can2motorRealInfo[i].TARGET_RPM,can2motorRealInfo[i].RPM);//速度环
                }
              //  pid_calc(&can1MOTOR_PID_RPM[i],can2motorRealInfo[i].TARGET_RPM,can2motorRealInfo[i].RPM);//速度环
                break;
            }
...
void Velocity_Planning_MODE(MOTOR_REAL_INFO *M3508_MOTOR)
{
    ...
    // 单圈模式
    if (M3508_MOTOR->FIRST_ANGLE_INTEGRAL_FLAG == 1)
    {
        // 过滤异常情况
        if(((M3508_MOTOR->Velocity_Planning.Pend > M3508_MOTOR->Velocity_Planning.Pstart) && (M3508_MOTOR->ANGLE < M3508_MOTOR->Velocity_Planning.Pstart)) ||		//[(结束位置 > 开始位置) && (处理过的真实角度pos <开始位置)]	||
        ((M3508_MOTOR->Velocity_Planning.Pend < M3508_MOTOR->Velocity_Planning.Pstart) && (M3508_MOTOR->ANGLE > M3508_MOTOR->Velocity_Planning.Pstart)))		//	[(结束位置 < 开始位置) && (处理过的真实角度pos >开始位置)]
        {
            M3508_MOTOR->TARGET_RPM = M3508_MOTOR->Velocity_Planning.Vstart;	//TARGET_RPM = 开始的速度
        }
        else if(((M3508_MOTOR->Velocity_Planning.Pend > M3508_MOTOR->Velocity_Planning.Pstart) && (M3508_MOTOR->ANGLE > M3508_MOTOR->Velocity_Planning.Pend)) ||
                ((M3508_MOTOR->Velocity_Planning.Pend < M3508_MOTOR->Velocity_Planning.Pstart) && (M3508_MOTOR->ANGLE < M3508_MOTOR->Velocity_Planning.Pend)))
        {
            M3508_MOTOR->TARGET_RPM = M3508_MOTOR->Velocity_Planning.Vstart;	//TARGET_RPM = 末尾的速度
        }
        else
        {
            S = ABS(M3508_MOTOR->ANGLE - M3508_MOTOR->Velocity_Planning.Pstart);      //开始位置
        ...
        //判断是否完成
        if((ABS(M3508_MOTOR->Velocity_Planning.Pend - M3508_MOTOR->ANGLE)) < 50)
        {
            M3508_MOTOR->Velocity_Planning.flag = 1;//设置标志位
            M3508_MOTOR->TARGET_RPM=0;
        }

        if((ABS(M3508_MOTOR->ANGLE - M3508_MOTOR->Velocity_Planning.Pend)) > 50)
        {
